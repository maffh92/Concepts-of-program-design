...
object MapObject {
  ...
  implicit object MapC extends Generic2[Map]{
    ...
    def idMap[A] : Map[A,A] = new Map[A,A]{def selMap(a : A) = id(a)}
    ...
    def product[A1,A2,B1,B2](ra : Map[A1,A2], rb : Map[B1,B2]) = {
      new Map[Product[A1,B1],Product[A2,B2]]{
        def selMap(product : Product[A1,B1]) = {
          product match{
    ...
    def view[A1,A2,B1,B2](iso1 : Iso[A2,A1],iso2 : Iso[B2,B1],ra: () => Map[A1,B1]) : Map[A2,B2] =
    {
      new Map[A2,B2]{
        override def selMap(a2: A2) : B2 = iso2.to(ra().selMap(iso1.from(a2)))
      }
    }
  }
